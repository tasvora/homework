...
with open(budgetData_csv_path, newline="") as csvfile:
    ...
    for row in range(0,len(budgetList)):
        ...
        if(row != len(budgetList)-1):
            ...
            # Adding an additional value to the master list and that will hold the average change between months.          
            budgetList[next].append(gain_loss_next-gain_loss_current)
            #Debugging reasons.
            #print(f' Next gain/loss {gain_loss_next}')
            #Resetting next back to 0, so that everytime we iterate just one row
            next = 0
    #Since after the entire process, the first list will not have an average change just putting a default 0 value.
